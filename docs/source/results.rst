...
- ``combined/combined.columns_filtered.pin`` - Present only when ``percolator_pin_columns_to_remove`` is set. This is the combined ``.pin`` file after removing the requested columns.
